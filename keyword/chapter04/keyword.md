...
코드 재사용성 및 가독성 향상을 통해 유지보수를 용이하게 할 수 있다는 장점도 있지만, AOP를 적용하면 핵심 로직에 명확하게 집중할 수 있어 코드의 의도를 파악하기가 쉬워진다.

예를 들어, 로그인 로직을 구현한다고 할 때, 로그인 유저의 정보를 받아서 DB 정보에서 조회를 하고 비교하고, 세션이나 쿠키를 관리하는 등의 핵심 로직도 있을 수 있지만, 로그인 처리를 감지하여 출석 체크를 해야하거나 포인트를 지급한다거나 하는 등 실제 로그인과 연관은 있어도 핵심이 아닌 부가적인 로직에 대한 처리를 인터셉터 등을 활용해서 분리하고 로그인 로직에서는 로그인의 핵심 로직에만 집중할 수 있도록 하는 등이 가능하다.
